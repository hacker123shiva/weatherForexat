...
const weatherApi = {
  ...
  // key: '01e14ff80462341592c573ea08a92509',
...
document.getElementById('search-button').addEventListener('click', function () {
  ...
  if (searchInputBox.value.trim() === '') {
    ...
    // Call the getWeatherReport function with the input box value
    ...
    weather_body.innerHTML = `
        ...
        <div class="card-body">
            <div class="location-details">
                <h5 class="card-title city text-dark" id="city">${
                  weather.name
                }, ${weather.sys.country}</h5>
                <p class="card-text date text-dark" id="date">${dateManage(
                  todayDate
                )}</p>
            </div>
            <div class="weather-status">
                <h2 class="card-title temp text-dark" id="temp">${Math.round(
                  weather.main.temp
                )}&deg;C</h2>
                <p class="card-text weather text-dark" id="weather">${
                  weather.weather[0].main
                } <i class="${getIconClass(weather.weather[0].main)}"></i></p>
                <p class="card-text min-max" id="min-max">${Math.floor(
                  weather.main.temp_min
                )}&deg;C (min) / ${Math.ceil(
      weather.main.temp_max
    )}&deg;C (max)</p>
                <p class="card-text text-dark" id="updated_on">Updated as of ${getTime(
                  todayDate
                )}</p>
            </div>
            <hr>
            <div class="day-details">
                <p class="card-text basic text-dark">Feels like ${
                  weather.main.feels_like
                }&deg;C | Humidity ${weather.main.humidity}% <br> Pressure ${
      weather.main.pressure
    } mb | Wind ${weather.wind.speed} KMPH</p>
            </div>
        </div>
    </div>
